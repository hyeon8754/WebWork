...
4. 웹브라우저에서 http://www.abc.com/abc.php 페이지를 접속했을 때 서버는 어떤 일을 수행하는가?<br/>
  4-1. DNS Resolution (도메인 이름 해석): 먼저, 브라우저는 "www.abc.com" 도메인을 IP 주소로 해석하기 위해 DNS 서버에 요청을 보낸다.<br/>
  4-2. 서버 연결: 브라우저는 얻어낸 IP 주소로 서버에 직접 연결한다.<br/>
  4-3. HTTP 요청 전송: 브라우저는 서버에게 해당 URL에 대한 HTTP GET 요청을 보낸다. 이 요청은 "abc.php"라는 파일을 요청하는 것이며, 이는 서버에서 PHP로 작성된 스크립트일   것이다.<br/>
  4-4. 서버에서의 PHP 스크립트 실행: 서버는 받은 요청을 해석하고, 서버 상에서 위치한 "abc.php" 파일을 찾는다. 이 파일은 PHP 코드를 포함하고 있으므로, PHP 엔진이 해당 스크립트를 실행한다.<br/>
